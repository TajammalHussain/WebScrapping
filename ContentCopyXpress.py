...
output_file = r'C:\Users\Tajammal\Desktop\MRF Brands upload\Xpress images\Contents.xlsx'
sheet_name = 'Sheet1'
...
def find_product_info(product_code):
    ...
    try:
        ...
        # Find the main content element on the product page
        main_content_element = driver.find_element(By.CSS_SELECTOR, 'div.product-catalog.tables#additionalInformation')
        main_content_html = main_content_element.get_attribute('innerHTML')
        
        # Use BeautifulSoup to parse the HTML content
        soup = BeautifulSoup(main_content_html, 'html.parser')
        
        # Extract data between <p> tags
        specifications_data = []
        
        for content in soup.find_all('div', class_='tables__subtable--content'):
            key = content.find('p', class_='tables__table--content--key').text.strip()
            value = content.find('p', class_='tables__table--content--value').text.strip()
            specifications_data.append(f"{key}: {value}")
        
        # Join specifications with a newline character
        description_text = "\n".join(specifications_data)
    except Exception as e:
...
print("Product links and and contents for the categorised products have been successfully updated in the Excel file. Interesting Upgrade")
print("The source of data we entered is related to scrapping(Data from Web) from the web, contains data for Pegler Yorkshire Products, nevertheless, the data is specific to particular products or range of products, and it does not have full record of Pegler Yorkshire Products.")